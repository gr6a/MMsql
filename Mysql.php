...
class MMysql {
	...
	protected $_host = 'localhost'; //地址
	protected $_port = 3306;
	protected $_user = 'user'; //数据库用户名
	protected $_pass = 'password'; //数据库密码
	protected $_dbName = 'dbname'; //数据库名
	...
	public function multiInsert($tbName, array $data) {
		$sql = "insert into " . $tbName . "(" . implode(',', array_keys($data[0])) . ") values";
		foreach ($data as $d) {
			$sql .= "('" . implode("','", array_values($d)) . "'),";
		}
		$sql = rtrim($sql, ',');
		$this->_doExec($sql);
		return self::$_dbh->lastInsertId();
		...
		if (is_string($option)) {
			...
			foreach ($option as $k => $v) {
				if (is_array($v)) {
					...
					$condition = ' (' . $this->_addChar($k) . ' ' . $relative . ' ' . $this->_addQuotes($v[0]) . ') ';
				} else {
					$logic = 'and';
					$condition = ' (' . $this->_addChar($k) . '=' . $this->_addQuotes($v) . ') ';
				}
	 ...
	 * @return string
	 */
	public function timeTrans($the_time) {
